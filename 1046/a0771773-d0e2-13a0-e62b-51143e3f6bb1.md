 ...
 **Observação**   Se você especificar que uma coluna deve ser ignorada, você deverá declarar explicitamente o tipo para todas as colunas restantes ou os dados não serão analisados corretamente. Se houver uma data reconhecível nos dados, a célula será formatada como uma data na planilha, mesmo se a configuração da coluna for Geral. Adicionalmente, se você especificar um dos formatos de data acima para uma coluna e os dados não contiverem uma data reconhecida, então o formato da célula na planilha será Geral.
